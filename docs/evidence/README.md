...
| [03 — `ash_agent` answers](03-ash-agent-answers.md) | `describe` (contract with types, descriptions, file-and-line), `validate` (camelCase key → `did_you_mean`; `"3"` → `3`; nothing run), `forbidden` (the policies that can deny `:discharge`, with the can-I-actually caveat) | None |
...
| [07 — Daemon slot](07-daemon-slot.md) | Nothing yet — **PENDING** on the parallel lane (DX-2) wiring `mix ash_agent.serve` into `.mcp.json`; placeholder checklist in place | The daemon's initialize/tools/call transcripts, once it mounts |
