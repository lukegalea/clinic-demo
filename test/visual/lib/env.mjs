...
export function chromiumOptions() {
  // Deliberately DEFAULT launch options: the rasterizer's glyph rounding is
  // part of the baseline contract (software-rendering flags change it by
  // ~1px per nav pill — enough to reflow the knife-edge row). Transient
  // capture failures are the capture() retry's job, not launch flags'.
